...
public:
    ...
    static vector<int> printMatrix(vector<vector<int> > matrix);            // 40. 顺时针打印矩阵
    static vector<vector<int>> permutation(vector<int> &nums);  // *** 51. 数字排列(含有重复数字) ***
    static int NumberOf1(int n);        // 26. 二进制中1的个数
};
